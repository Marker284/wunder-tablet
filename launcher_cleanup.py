...
def install_mdm(adb: Adb, log: Log, args: argparse.Namespace, apk: str,
    ...
    if "INSTALL_FAILED_UID_CHANGED" in last or "shared user changed" in last:
        ...
        log.info("частая причина: в новой сборке из манифеста пропал "
                 "android:sharedUserMaxSdkVersion. С ним на Android 13+ агент "
                 "ставится вообще без shared user, без него — требует его "
                 "завести, а менять это на установленном пакете нельзя")
        log.info("что делать: запросить у Headwind сборку, где "
                 "sharedUserMaxSdkVersion на месте (как в той, что уже стоит), "
                 "либо обновлять агент через саму консоль MDM, а не через adb")
